...
recipe_ingredients_strategy = st.lists(
    st.text(min_size=1, max_size=100), min_size=0, max_size=20
)
...
def test_get_random_recipe_endpoint_fuzzing(recipe):
    ...
    with patch(
        "src.services.api_service.APIService.get_random_recipe"
    ) as mock_get_random:
...
def test_search_recipes_endpoint_fuzzing(
    name,
    ingredient,
    include_ingredients,
    exclude_ingredients,
    ...
    with patch(
        "src.services.api_service.APIService.search_recipes_by_name"
    ) as mock_search_by_name:
        with patch(
            "src.services.api_service.APIService.search_recipes_by_ingredient"
        ) as mock_search_by_ingredient:
...
@settings(suppress_health_check=[HealthCheck.filter_too_much, HealthCheck.too_slow])
@given(recipes=st.lists(recipe_object_strategy, min_size=0, max_size=5))
...
@settings(suppress_health_check=[HealthCheck.filter_too_much, HealthCheck.too_slow])
@given(recipe_id=recipe_id_strategy, user_id=user_id_strategy, success=st.booleans())
